      ...
      <div className="card-premium relative overflow-hidden rounded-2xl page-header-hero">
        ...
        <div className="relative flex items-center gap-3 px-6 py-5">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "linear-gradient(135deg, hsl(var(--section-hsl, 262 80% 62%) / 0.30), hsl(var(--section-hsl, 262 80% 62%) / 0.08))", border: "1px solid hsl(var(--section-hsl, 262 80% 62%) / 0.22)" }}>
            <Search className="h-5 w-5 drop-shadow-[0_0_6px_currentColor]" style={{ color: "hsl(var(--section-hsl, 262 80% 62%))" }} />
          </div>
          <div>
            <h1 className="text-xl font-black tracking-tight leading-tight gradient-text">Búsqueda global</h1>
          ...
          {totalCount === 0 ? (
            <div className="text-center py-10 text-muted-foreground">
              <div
                ...
                style={{
                  background: "linear-gradient(135deg, hsl(var(--section-hsl, 192 75% 50%) / 0.20), hsl(var(--section-hsl, 192 75% 50%) / 0.07))",
                  border: "1px solid hsl(var(--section-hsl, 192 75% 50%) / 0.22)",
                  boxShadow: "0 8px 32px hsl(0 0% 0% / 0.15)"
                }}
              >
                <Search className="h-8 w-8" style={{ color: "hsl(var(--section-hsl, 192 75% 50%))" }} />
              </div>
            ...
            <>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-all active:scale-95",
                          isActive
                            ? "bg-secondary border-border text-foreground font-black ring-1 ring-inset ring-border"
              ...
              {groupsWithIndex.map(({ type, items }) => {
                ...
                const Icon = meta.icon;
                return (
                  <div key={type} className="card-premium rounded-2xl overflow-hidden">
                    {/* Section header */}
                    ...
                    <div className="divide-y divide-border/50">
                      {items.map((result) => (
                        <Link
                          ...
                          className={cn(
                            "row-interactive flex items-center gap-3 px-4 py-3 transition-all active:scale-[0.99] group",
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{highlightText(result.title, query)}</p>
                            {result.subtitle && (
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                <a key={href} href={href}
                  className="card-premium flex items-start gap-3 p-3.5 rounded-2xl hover:-translate-y-0.5 hover:shadow-[0_8px_24px_hsl(0_0%_0%/0.25)] transition-all active:scale-[0.99] group">
